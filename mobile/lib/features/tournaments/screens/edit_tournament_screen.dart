...
class _EditTournamentScreenState extends ConsumerState<EditTournamentScreen> {
  ...
  void initState() {
    ...
    _cityCtrl = TextEditingController(text: t.city ?? '');
    _descCtrl = TextEditingController(text: t.description ?? '');
    _maxParticipantsCtrl = TextEditingController(
      text: t.maxParticipants?.toString() ?? '',
    );
    _format = t.format;
    _matchFormat = t.matchFormat;
    _playType = t.playType;
    _registrationDeadline = t.registrationDeadlineDate?.toLocal();
    _startsAt = t.startsAtDate?.toLocal();
  ...
  Future<void> _pickDate({required bool isRegistration}) async {
    ...
    final initial =
        isRegistration ? (_registrationDeadline ?? now) : (_startsAt ?? now);
  ...
  Future<void> _submit() async {
    ...
    final ok = await ref
        .read(tournamentDetailProvider(widget.tournament.id).notifier)
...
class _DatePickerTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final formatted =
        local != null ? '${local.day}/${local.month}/${local.year}' : 'Not set';
                ...
                Expanded(
                  child: Text(
                    ...
                    style: theme.textTheme.bodySmall?.copyWith(
                      color:
                          hasDate ? AppColors.onSurface : AppColors.disabled,
                      fontWeight:
