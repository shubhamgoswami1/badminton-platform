...
class _EditLogScreenState extends ConsumerState<EditLogScreen> {
  ...
  void initState() {
    ...
    _sessionType = widget.log.sessionType;
    _intensity = widget.log.intensity;
  }
  ...
  Future<void> _submit() async {
    ...
    final ok = await ref.read(trainingLogsProvider.notifier).updateLog(
          widget.log.id,
          TrainingLogCreate(
...
// ── Selectors (reused from AddLogScreen) ─────────────────────────────────────
...
class _IntensitySelector extends StatelessWidget {
  const _IntensitySelector({required this.selected, required this.onChanged});
  final String? selected;
  final ValueChanged<String?> onChanged;
    ...
    return Wrap(
      ...
      children: [
        ChoiceChip(
          ...
          side: BorderSide(
            color: selected == null ? AppColors.onSurfaceVariant : AppColors.outline,
          ),
